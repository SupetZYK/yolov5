...
def train(hyp, opt, device, tb_writer=None):
    ...
    test_json_path, test_img_path = data_dict['val']
    ...
    for k, v in model.named_modules():
        ...
        elif hasattr(v, 'scale') and isinstance(v.scale, nn.Parameter):
            pg0.append(v.scale) # apply no decay to scale parameter
    ...
    # Trainloader
    dataloader, dataset = create_dataloader(train_json_path, train_img_path, imgsz, batch_size, gs, opt,
    ...
    # Process 0
    if rank in [-1, 0]:
        testloader = create_dataloader(test_json_path, test_img_path, imgsz, batch_size // 2, gs, opt,  # testloader
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            loss_func = model.module.loss if hasattr(model, 'module') else model.loss
            losses = loss_func(cls_preds, loc_preds, centerness, targets.to(device))
            ...
            if rank in [-1, 0]:
                ...
                if plots and ni < 3:
                    ...
                    Thread(target=plot_images, args=(imgs * img_std + img_mean, targets, paths, f), daemon=True).start()
                    # if tb_writer:
