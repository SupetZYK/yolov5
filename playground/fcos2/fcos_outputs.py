...
class FCOSOutputs(nn.Module):
    def __init__(self):
        ...
        self.loc_loss_func = IOULoss('giou')
    ...
    def _get_ground_truth(self, locations, gt_instances, batch_size):
        ...
        for i in range(batch_size):
            new_targets.append(gt_instances[gt_instances[:,0]==i][:,1:])
        training_targets = self.compute_targets_for_locations(
            locations, new_targets, loc_to_size_range, num_loc_list
        )

        training_targets["locations"] = [locations.clone() for _ in range(len(new_targets))]
        training_targets["im_inds"] = [
            locations.new_ones(locations.size(0), dtype=torch.long) * i for i in range(len(new_targets))
        ]
    ...
    def compute_targets_for_locations(self, locations, targets, size_ranges, num_loc_list):
        ...
        for im_i in range(len(targets)):
            ...
            labels_per_im = targets_per_im[:,0].long()
    ...
    def losses(self, logits_pred, reg_pred, ctrness_pred, locations, gt_instances, top_feats=None):
        ...
        """
        bs = logits_pred[0].shape[0]
        training_targets = self._get_ground_truth(locations, gt_instances, bs)
    ...
    def fcos_losses(self, instances):
        ...
        if self.box_quality == "ctrness":
            ...
            losses["center_loss"] = ctrness_loss
